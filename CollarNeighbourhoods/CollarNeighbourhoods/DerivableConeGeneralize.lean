...
variable {𝕜 E : Type*} [NontriviallyNormedField 𝕜] [NormedAddCommGroup E]
  [NormedSpace 𝕜 E] [LinearOrder 𝕜] [PosMulReflectLT 𝕜] [IsStrictOrderedRing 𝕜]
  [OrderTopology 𝕜]
...
def decIndexSeq' {Q : ℕ → 𝕜 → Prop}
    (hγn : ∀ (n : ℕ), ∃ i ∈ Ioc (0 : 𝕜) (1 / (n + 1)), ∀ j ∈ Ioc 0 i, Q n j) :
    ℕ → 𝕜
  | 0 => Classical.choose (hγn 0)
  | n + 1 => min (decIndexSeq' hγn n / 2) (Classical.choose (hγn (n + 1)))

lemma indexSeq_mem_ioc' {Q : ℕ → 𝕜 → Prop}
    (hγn : ∀ (n : ℕ), ∃ i ∈ Ioc (0 : 𝕜) (1 / (n + 1)), ∀ j ∈ Ioc 0 i, Q n j) (n : ℕ) :
    decIndexSeq' hγn n ∈ Ioc (0 : 𝕜) (1 / (n + 1)) := match n with
...
lemma indexSeq_pos' {Q : ℕ → 𝕜 → Prop}
    (hγn : ∀ (n : ℕ), ∃ i ∈ Ioc (0 : 𝕜) (1 / (n + 1)), ∀ j ∈ Ioc 0 i, Q n j) (n : ℕ) :
    0 < decIndexSeq' hγn n :=
  (indexSeq_mem_ioc' hγn n).1

omit [PosMulReflectLT 𝕜] [IsStrictOrderedRing 𝕜] in
lemma indexSeq_prop' {Q : ℕ → 𝕜 → Prop}
    (hγn : ∀ (n : ℕ), ∃ i ∈ Ioc (0 : 𝕜) (1 / (n + 1)), ∀ j ∈ Ioc 0 i, Q n j) (n : ℕ) :
...
lemma antitone_decIndexSeq' {Q : ℕ → 𝕜 → Prop}
    (hγn : ∀ (n : ℕ), ∃ i ∈ Ioc (0 : 𝕜) (1 / (n + 1)), ∀ j ∈ Ioc 0 i, Q n j) :
...
lemma tendsto_decIndexSeq' {Q : ℕ → 𝕜 → Prop}
    (hγn : ∀ (n : ℕ), ∃ i ∈ Ioc (0 : 𝕜) (1 / (n + 1)), ∀ j ∈ Ioc 0 i, Q n j) :
  ...
  intro m hm
  simp
  have := antitone_decIndexSeq' hγn hm
  have := (indexSeq_mem_ioc' hγn n).2
  sorry

lemma slope_mem_ball' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) (n : ℕ) :
    ∃ i ∈ Ioc (0 : 𝕜) (1 / (n + 1)), ∀ j ∈ Ioc 0 i,
  ...
  specialize this hγn
  rw [mem_map, mem_nhdsGT_iff_exists_Ioc_subset] at this
...
lemma exists_of_eventually_mem' {γ : ℕ → 𝕜 → E} {s : Set E} (n : ℕ)
    (hs : ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) :
    ∃ i ∈ Ioc (0 : 𝕜) (1 / (n + 1)), ∀ j ∈ Ioc 0 i,
      γ n j ∈ s := by
...
lemma exists_slope_and_mem' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E} {n : ℕ}
    (hs : ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) :
    ∃ i ∈ Ioc (0 : 𝕜) (1 / (n + 1)), ∀ j ∈ Ioc 0 i,
...
lemma slope_mem_decIndexSeq' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E} {n : ℕ}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) {j : 𝕜}
...
lemma mem_set_decIndexSeq' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E} {n : ℕ}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) {j : 𝕜}
    (hj : j ∈ Ioc 0 (decIndexSeq' (fun n ↦ exists_slope_and_mem' hγv hγ (hs n)) n)) :
...
open Classical in
def limitFun' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s)
    (p : E) (x : 𝕜) : E :=
...
lemma limitFun_of_not_mem_Ioc' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) (p : E) {x : 𝕜}
...
lemma limitFun_zero' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) (p : E) :
    limitFun' hγv hγ hs p 0 = p := by
  apply limitFun_of_not_mem_Ioc'
  exact left_notMem_Ioc

def limitFunPoint' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) {x : 𝕜}
...
lemma mem_Ioc_limitFunPoint' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) {x : 𝕜}
    (hx : x ∈ Ioc 0 (decIndexSeq' (fun n ↦ exists_slope_and_mem' hγv hγ (hs n)) 0)) :
...
lemma eq_limitFunPoint_of_mem_Ioc' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) {x : 𝕜}
...
lemma limitFun_eq_limitFunPoint' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) (p : E) {x : 𝕜}
    (hx : x ∈ Ioc 0 (decIndexSeq' (fun n ↦ exists_slope_and_mem' hγv hγ (hs n)) 0)) :
...
lemma limitFun_mem' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {p : E} {s : Set E} (hps : p ∈ s)
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) (x : 𝕜) :
...
lemma hasDerivWithinAt_limitFun' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {p : E} {s : Set E}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s)
    (hγp : ∀ (n : ℕ), γ n 0 = p)
...
lemma differentiableWithinAt_limitFun' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) {p : E}
    (hγp : ∀ (n : ℕ), γ n 0 = p)
    {w : E} (hv : Tendsto v atTop (𝓝 w)) :
    DifferentiableWithinAt 𝕜 (limitFun' hγv hγ hs p) (Ici 0) 0 :=
  (hasDerivWithinAt_limitFun' hγv hγ hs hγp hv).differentiableWithinAt


lemma derivWithin_limitFun' {γ : ℕ → 𝕜 → E} {v : ℕ → E}
    (hγv : ∀ (n : ℕ), (fderivWithin 𝕜 (γ n) (Ici 0) 0) 1 = v n)
    (hγ : ∀ n, DifferentiableWithinAt 𝕜 (γ n) (Ici 0) 0) {s : Set E}
    (hs : ∀ n, ∀ᶠ (x : 𝕜) in 𝓝[≥] 0, γ n x ∈ s) {p : E}
    (hγp : ∀ (n : ℕ), γ n 0 = p)
    {w : E} (hv : Tendsto v atTop (𝓝 w)) :
    derivWithin (limitFun' hγv hγ hs p) (Ici 0) 0 = w := by
  apply (hasDerivWithinAt_limitFun' hγv hγ hs hγp hv).derivWithin ?_
