...
-- I think you need to proof this via Whitney extension theorem
theorem ContDiffWithinAt.exists_mem_nhds_contDiffAt {F : Type*} [NormedAddCommGroup F]
    [NormedSpace ℝ F] (n : WithTop ℕ∞)
    (f : E → F) (s : Set E) (x : E) (hf : ContDiffWithinAt ℝ n f s x) :
    ∃ g : E → F, EqOn f g s ∧ ContDiffAt ℝ n g x  := by
  sorry
...
theorem exists_isMIntegralCurveAt_of_contMDiffAt' [CompleteSpace E]
  ...
  obtain ⟨_, hv⟩ := hv
  obtain ⟨g, hgv, hg⟩ := hv.exists_mem_nhds_contDiffAt
  ...
  have h : HasDerivAt f (g (f t)).2 t := (haux t ht).1
  have hf3 : f t ∈ (extChartAt I x₀).target := (haux t ht).2
